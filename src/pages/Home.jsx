...
import mainBanner2 from '../assets/img/banner/DonTetSaleHet1.png';
...
function Home() {
  ...
  useEffect(() => {
    const filterHotSalesProducts = products.filter(
      (item) => item.displayHome === 'hotSale'
    );
    setHotSalesProducts(filterHotSalesProducts);

    const filterBestSalesProducts = products.filter(
      (item) => item.displayHome === 'bestSale'
    );
    setBestSalesProducts(filterBestSalesProducts);

    const filterNewProducts = products.filter(
      (item) => item.displayHome === 'new'
    );
    setNewProducts(filterNewProducts);

    const filterRecommendedProducts = products.filter(
      (item) => item.displayHome === 'recommended'
    );
            ...
            <div className='col-span-3 max-lg:col-span-full'>
              <div className='relative shadow-s0'>
                <label
                  ...
                  className='category-btn max-md:h-[46px]'
                >
                ...
                <div className='category__list h-0 overflow-hidden'>
                        ...
                        <div className='category__child'>
                          {item.childCats.map((childCat, index) => (
                            <div
                              key={index}
                              className={
                                childCat.title === '' ? 'hidden' : 'w-[180px]'
                              }
                            >
                              <div className='py-[10px] px-[12px]'>
                                <span className='category__child--head'>
                                  {childCat.title}
                                </span>
                              </div>
                              <ul className='list-none'>
                                {childCat.childItems.map((childItem, index) => (
                                  <li key={index} className='px-[12px]'>
                                    <a
                                      className='text-16 py-[10px] block hover:text-primary hover:font-medium'
                                      href={childItem.href}
                                    >
                                      {childItem.name}
                                    </a>
              ...
              <div className='bg-primary h-auto max-md:h-auto rounded overflow-hidden '>
                {/* <div className='text-white cursor-default col-span-1 px-5 pt-16 pb-4 max-md:col-span-full'>
                  <span className='text-16 font-light'>
                    ...
                    Đến với website chính thức của
                  </span>
                  <h2 className='text-[3.2rem] max-md:text-24 my-6'>
                    Cty tnhh Hiền Phát Vina
                  </h2>
                  <p className='text-16 font-light'>
                    Chuyên cung cấp gas, bếp gas, điện gia dụng, máy lọc nước,
                    máy làm mát, gạo, tấm
                  </p>
                  <button className='buy__btn bg-white text-primary mt-20'>
                    <Link to='/shop'>Đến mua ngay!</Link>
                  </button>
                </div>
                <div className='col-span-1 p-4 text-right max-md:col-span-full flex justify-center items-center'>
                  <div className='relative'>
                    ...
                    <a
                      className='text-333 text-sm absolute right-1/4 bottom-10'
                      target='_blank'
                      href='http://www.freepik.com'
                    >
                      Designed by alexandercho / Freepik
                    </a>
                  </div>
                </div> */}
                ...
                <span key={index} className='product__cat-item'>
                  <Link to={product.href} className='product__cat-item-link'>
                    <img
                      src={product.src}
                      alt=''
                      className='w-[80%] block bg-[#f8f9fa] rounded-[50%]'
                    />
          ...
          <div className='wid-1200'>
            <div className='counter grid grid-cols-12'>
              <div className='col-span-6 max-md:col-span-full'>
                <div>
                  <h4 className='text-white text-20 mt-8 mb-4'>
                    Ưu đãi giới hạn
                  </h4>
                  <h3 className='text-white text-24 mb-4'>
                    Bếp gas Paloma nhập khẩu Nhật Bản
                  </h3>
                </div>
                <Clock />
                <button className='buy__btn bg-white text-primary mt-10'>
                  <Link to='/shop'>Đến xem ngay!</Link>
                </button>
              </div>
              ...
              {blogItems.slice(0, 3).map((blogItem, index) => (
                <div key={index} className='col-span-4 max-md:col-span-12'>
                  <div className='text-14'>
                    ...
                    <span className='blog-desc overflow-hidden list-disc'>
                      {blogItem.blogDesc}
                    </span>
                  </div>
                </div>
